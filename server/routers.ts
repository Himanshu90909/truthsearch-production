...
export const appRouter = router({
  ...
  research: router({
    ...
    start: publicProcedure.input(z.object({ question: z.string().trim().min(8).max(1200), contextText: z.string().max(60000).optional(), imageUrls: z.array(z.string().url().max(600)).max(4).optional() })).mutation(async ({ input, ctx }) => { const id = await createSession(input.question, ctx.user?.id); const research = runResearch(id, input.question, { contextText: input.contextText, imageUrls: input.imageUrls }); if (SYNC_RESEARCH) await research; else void research; return { id }; }),
