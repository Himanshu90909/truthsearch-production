...
export default function Home() {
  ...
  return <div className={dark ? "app dark" : "app"}>
    ...
    {!sessionId ? <main className="landing" id="research">
      ...
      <div className="workspace-grid"><aside className="workspace-sidebar"><button className="sidebar-new" onClick={() => { setSessionId(null); setQuestion(""); setFollowups([]); }}><Sparkles size={15} /> New research</button><div className="sidebar-section"><div className="eyebrow">Recent research</div><p className="sidebar-empty">Your completed research will appear here.</p></div><div className="sidebar-section"><div className="eyebrow">RL learning</div>{(() => { const s = rlSummary(); return s.samples > 0 ? <p className="sidebar-empty">{s.samples} feedback {s.samples === 1 ? "sample" : "samples"} · best source: {s.top}</p> : <p className="sidebar-empty">Rate answers with Helpful / Not helpful — the app learns which sources to trust.</p>; })()}</div><Progress data={data} latestProgress={latestProgress} activeStage={activeStage} />{data?.session?.plan?.queries?.length > 0 && <div className="mini-card"><div className="eyebrow">Search plan</div>{data.session.plan.queries.map((q: string) => <p key={q}>{q}</p>)}</div>}</aside><section className="results thread">
